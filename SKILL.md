...
> ⚠️ AI토크 날짜와 주제는 기수관리 테이블의 `AI토크_Day1/Day2/Day3` + `AI토크_Day1_주제/Day2_주제/Day3_주제` 필드에서 동적으로 가져온다.
> 해당 필드가 모두 비어있으면 이 Phase를 건너뛴다 (AI토크가 없는 기수).

**레벨 기반 자동 매칭**: 스터디장이 Phase 3에서 선택한 레벨과 `AI토크_DayN_주제`를 매칭하여, 해당 Day의 날짜를 자동으로 안내한다.

```
[스터디장 레벨]에 해당하는 AI토크가 [매칭된 Day 날짜 + 요일]에 있어요!
시간: 21:00~23:00
...
선택 결과를 Airtable의 `AI토크 가능여부` 필드에 저장. (예: "5/6(수) 입문 — 참석")
...
`,
  qaRaw: "Q&A 원본 텍스트",
  aiTalkAvailability: "5/6(수) 입문 — 참석",
  studyDays: "화요일, 수요일",
  offlineMeeting: "예",
};
