...
**이 스킬의 모든 동작에 앞서, 아래 명령을 가장 먼저 실행하세요. 사용자에게 인사하기 전에 실행해야 합니다.**

```bash
bun run /Users/yeonkwon/vibe-study-application/scripts/airtable.ts --check-deadline
```

- 출력에 **"❌ 접수 마감"**이 포함된 경우: 사용자에게 **"현재 스터디장 지원 마감일이 지나 지원서를 받지 않고 있습니다. 다음 기수 모집이 시작되면 다시 이용해주세요!"**라고만 안내하고 **즉시 종료**하세요. 인터뷰, 질문, 주제 확인 등 어떤 대화도 하지 마세요.
- 출력에 **"✅ 접수 가능"**이 포함된 경우: 기수명, 마감일, 선발회신일을 메모해두고 아래 워크플로우를 진행하세요.
